...
.controller('reportCtrl',function($scope, $window, $location, reportService) {
    $scope.report = function(id) {
        ...
        if($scope.reportMessage){
            if($scope.reportMessage.length >= 500 || $scope.reportMessage.length < 1){
                $scope.reportWarning = "Your report has to be between one and 500 chracters long";
            }else{
                reportService.reportPost(id,$scope.reportMessage).then(function(response){
                    if(response.success){
                        $scope.reportSuccess = "Your report has been sent. Thank you for your input.";
                    }else{
